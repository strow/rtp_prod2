function [head, hattr, prof, pattr] = create_cris_nc_random_day_rtp(fnCrisInput, cfg)
...
fnLst1 = dir(fullfile(fnCrisInput, 'SNDR.*.nc')); 
...
% $$$     try
        [h,ha,p,pa] = uwnc2rtp(fpath, cfg);
% $$$     catch
% $$$         fprintf(2, ['>>>> ERROR: uwnc2rtp failure reading %s.\n\t\tTrying ' ...
% $$$                     'next granule\n'], fpath);
% $$$         continue;
% $$$     end
    if length(p.rtime) == 0
...
sarta_cfg.sartaclear_code = sartaclr_exec;
