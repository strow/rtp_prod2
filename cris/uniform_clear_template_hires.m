function fx = uniform_clear_template_hires(fin,fout,sumout);
...
%proxy_box_to_ham_hires    %% should produce prof.robs1 identical to rad_ham
%final_robs = prof.robs1;
...
head0 = head;
prof0 = prof;
[head0x, prof0x] = subset_rtp(head0, prof0, [], idtestc, []);
%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% comment this out for now, assume rcalc already there
%%%%% >>>>>>>>>>>>>>>>
%{
...
disp('re-loading original RTP data, to get "box" apodized robs1')
[head, hattr, prof, pattr] = rtpread(RTPIN);
...
figure(2); 
tobs = rad2bt(ff,prof.robs1(1:2223,:)); tobs = real(tobs);
tcal = rad2bt(ff,prof.rcalc(1:2223,:)); tcal = real(tcal);
