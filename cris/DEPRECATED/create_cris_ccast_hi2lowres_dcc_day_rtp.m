function [head, hattr, prof, pattr] = create_cris_ccast_hi2lowres_dcc_day_rtp(fnCrisInput, cfg)
...
% above:
func_name = 'create_cris_ccast_hi2lowres_dcc_day_rtp';
...
fprintf(1, '>> %s Running create_cris_ccast_lowres_dcc_day_rtp for input: %s\n', ...
...
for i=1:filestride:length(fnLst1)
    ...
    fprintf(1, '>>> Granule %d\n', i);
% $$$     try
        [h, ha, p, pa] = ccast2rtp_hi2lo(fullfile(fnLst1(i).folder,fnLst1(i).name), cfg.nguard);
% $$$     catch
