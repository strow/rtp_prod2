function [head, hattr, prof, pattr] = create_cris_ccast_hi2lowres_random_day_rtp(fnCrisInput, cfg)
...
% above:
func_name = 'create_cris_ccast_hi2lowres_random_day_rtp';
...
for i=1:numel(fnLst1)
    ...
    fprintf(1, '>>> Granule %d\n', i);
    try
        [h, ha, p, pa] = ccast2rtp_hi2lo(fullfile(fnCrisInput,fnLst1(i).name), nguard);
    catch
