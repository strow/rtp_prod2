function [head, hattr, prof, pattr] = create_cris_hires_dcc_rtp(inpath, cfg)
...
% Process a single CrIS hires .mat granule file.
    ...
    for i=1:stride:length(files)
        ...
        ftest = h_gran.vchan(indtest);
        r = p_gran.robs1(indtest,:);
        ibad = find(r < 1E-6);
        r(ibad)=1E-6;
        mbt = mean(rad2bt(ftest,r),1); % [1 x nobs]
        ...
        p2 = rtp_sub_prof(p_gran, dcc_ind);
        p_gran = p2;
