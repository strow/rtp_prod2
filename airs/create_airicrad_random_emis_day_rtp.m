function [head, hattr, prof, pattr] = create_airicrad_random_emis_day_rtp(inpath, cfg)
%
% NAME
...
for i=1:length(files)
    ...
    if i == 1 % only need to build the head structure once but, we do
        ...
        p = uniform_random_select(prof0,cfg);  % select for
                                                 % random/nadir obs
...
prof.l1cproc = prof.l1cproc(keepchaninds,:);
prof.l1csreason = prof.l1csreason(keepchaninds,:);
