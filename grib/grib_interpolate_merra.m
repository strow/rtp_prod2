...
% merra data collection lat/lon grid is already monotonically
% increasing so, no need for flipud like fill_era or fill_ecmwf

F.sp.ig   = griddedInterpolant(X,Y,single(ncread(fn_s,'sp',[1 1 hindex],[Inf Inf 1]))'/100,'linear');
F.skt.ig  = griddedInterpolant(X,Y,single(ncread(fn_s,'skt',[1 1 hindex],[Inf Inf 1]))','linear');
F.v10.ig  = griddedInterpolant(X,Y,single(ncread(fn_s,'v10',[1 1 hindex],[Inf Inf 1]))','linear');
F.u10.ig  = griddedInterpolant(X,Y,single(ncread(fn_s,'u10',[1 1 hindex],[Inf Inf 1]))','linear');
F.tcc.ig  = griddedInterpolant(X,Y,single(ncread(fn_s,'tcc',[1 1 hindex],[Inf Inf 1]))','linear');
F.ci.ig   = griddedInterpolant(X,Y,single(ncread(fn_s,'ci',[1 1 hindex],[Inf Inf 1]))','linear');
